...
RSpec.describe Event do
  ...
  it 'can list its food trucks by name' do
    expect(@event.food_truck_names).to eq(["Rocky Mountain Pies", "Ba-Nom-a-Nom", "Palisade Peach Shack"])
  end

  it 'can list food trucks that sell a particular item' do
    expect(@event.food_trucks_that_sell(@item1)).to eq([@food_truck1, @food_truck3])
    expect(@event.food_trucks_that_sell(@item4)).to eq([@food_truck2])
  end

  it 'can report a total inventory' do
  ...
  it 'can report overstocked itmes' do
    expect(@event.overstocked_items).to eq([@item1])
  end

  it 'can list all items in stock alphabetically by name' do
...
RSpec.describe Event do
  ...
  # it 'has a date' do
  #   #Not really sure how to stub... Could use #freeze_time?
